...
fin = time.time()
print(f'Tiempo usado por SARSA con {episodes} episodios = {round(fin - ini,2)} segundos')

# Después de terminar la simulación guardar la data entrenada
with open('s_table_cartpole_v1.pkl', 'wb') as f:
...
plt.title('SARSA - Recompensa total por episodio')
plt.grid(True)
...
plt.title('SARSA - Simulación con el agente entrenado')
plt.subplot(2, 1, 1)
